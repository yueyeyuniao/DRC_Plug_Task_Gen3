...
class SurpriseTask
{
  public:
    ...
    void getEEPose_pre_grasp_pullout()
    {
      // get the position of the wire
        ...
        target_pose_wire1.position.x = transform_wire.getOrigin().x()-0.19;
        ...
        target_pose_wire2 = target_pose_wire1;
        target_pose_wire2.position.x = target_pose_wire1.position.x+0.04;
  ...
  /////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
      try{
          listener_.waitForTransform("end_effector_link", "preinsert",ros::Time(0), ros::Duration(3.0));
          listener_.lookupTransform("end_effector_link", "preinsert",ros::Time(0), transform_pre_ee);
      }
      catch (tf::TransformException ex) {
          ROS_ERROR("%s",ex.what());
      }
    ...
    void getXYZ_gen3_base()
    {
      try{
          listener_.waitForTransform("base_link", "preinsert",ros::Time(0), ros::Duration(3.0));
          listener_.lookupTransform("base_link", "preinsert",ros::Time(0), transform_pre_world);
      }
      catch (tf::TransformException ex) {
          ROS_ERROR("%s",ex.what());
      }

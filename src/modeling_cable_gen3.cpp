...
class PointCloudProc
{
  public:
    ...
    label:
        ...
        listener_.waitForTransform("/base_link", "finger",ros::Time(0), ros::Duration(3.0)); // if not using trajopt, change "finger" to be "gripper_finger1_finger_tip_link"
        listener_.lookupTransform("/base_link", "finger",ros::Time(0), transform); // if not using trajopt, change "finger" to be "gripper_finger1_finger_tip_link"
        ...
        if (isOut == false){
          pass.setFilterLimits(transform.getOrigin().x()+0.1, center_object.x + 0.61);
        }
        else {
          pass.setFilterLimits(transform.getOrigin().x()+0.055, center_object.x + 0.61);
        }
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
        pass.setFilterFieldName("y");
        pass.setFilterLimits(center_object.y - 0.1, center_object.y + transform.getOrigin().y()+0.4); // 0.07 needs to change according to the ee's position
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
        pass.setFilterFieldName("z");
        pass.setFilterLimits(center_object.z + 0.25, center_object.z + 0.7);  //0.35 is for testing disturbance
        pass.filter(*cloud_filtered_);
    ...
    void CurveFit(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
      ...
      av.clear();
      bb = PolyfitXY(xx,zz);  //z = bb[0]*x^0 + bb[1]*x^1 + bb[2]*x^2
...
//      }
      ...
      if ((tip_temp != 0.0) && (0.61-tip_temp) > 0.004) {
        isOut = true;
      }
      ...
      if (N>8000){
        ...
        num_sample = floor(N/600);
      }

      //int num_sample = floor(N/700);   // 1000 -> power cable  700 -> hdmi
      double segment = (tip_temp-end_temp)/num_sample;
